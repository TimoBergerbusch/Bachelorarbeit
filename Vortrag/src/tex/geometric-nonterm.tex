...
%\begin{frame}
		...
		\item \red{Problem}: $g$ k\"onnte nicht die Form $\varphi \le c$ haben
		\item \red{Schlimmer}: $g$ k\"onnte mittels "$=$" neue Variablen einf\"uhren
		\item \green{L\"osung}: umformen von $g$ in die gew\"unschte Form: 
			\begin{tabular}{cll}
...
\begin{frame} % bsp: filter eq.
	\begin{example}
		F\"urs Beispiel \its erhalten wir:\newline
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item[] \green{L\"osung}: Substitutionen auf die Updates anwenden
		\item \red{Problem}: keine Struktureigenschaft wie bei den Guards
		\item[] \green{L\"osung}: rekursive Suche mit zwei Eigenschaften:
			\begin{enumerate}
...
\begin{frame} % iterationmatrix
	...
	\begin{itemize}
		...
		\item Iteration Matrix \& Konstanten gegeben durch: 
			\begin{figure}[H]
				\centering
...
\begin{frame}
	\begin{variableblock}{Erinnerung: Hergeleitete Matrix und Werte}{bg=orange!50!white,fg=black}{bg=orange, fg=white}
		...
		\end{tabular}
	\end{variableblock}
	\begin{example}[Bedingungen \rom{1}: Point Crit.]
		\begin{itemize}
...
\begin{frame}
	\begin{variableblock}{Erinnerung: Hergeleitete Matrix und Werte}{bg=orange!50!white,fg=black}{bg=orange, fg=white}
		...
		\end{tabular}
	\end{variableblock}
	\begin{example}[Bedingungen \rom{2}: Ray Crit.]
		\begin{itemize}
...
\begin{frame}
	...
	\end{itemize}
	\begin{example}[Verifizierung eines GNA \rom{1}]
...
\begin{frame}
	\begin{example}[Verifizierung eines GNA \rom{2}]
		Vom \solver: $y_1=\begin{pmatrix} 9 \\ 0 \end{pmatrix}$, $y_2=\begin{pmatrix} 8 \\ -8 \end{pmatrix}$, $\mu_1=0$\newline
		\begin{itemize}
