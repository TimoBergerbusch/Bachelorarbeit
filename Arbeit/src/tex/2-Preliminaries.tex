...
As stated in \autoref{sec:aprove} \aprove can handle programs of different languages. By transforming these into a \textit{Symbolic Execution Graph} one has a common form for all programs. %TODO: genaueres über die transformierung? Quelle zur äquivalenz?
...
\begin{figure}[H]
	\vspace{3em}
	%TODO: redo with braces
	\begin{lstlisting}[escapechar=!]
		!$f_x\tikz[remember picture] \node [] (sfs) {};\rightarrow f_y \tikz[remember picture] \node [] (fs) {};(v_1, \dots v_n) :|: cond_1$! !\tikz[remember picture] \node [] (cond1) {};!
		!$f_y(v_1, \tikz[remember picture] \node [] (var1) {};\dots v_n) \rightarrow f_y(v^\prime_1, \tikz[remember picture] \node [] (var2) {}; \dots v^\prime_n)  :|: cond_2$\tikz[remember picture] \node [] (cond2) {};! !\tikz[remember picture] \node [] (lr) {};!
	\end{lstlisting}
	\begin{tikzpicture}[remember picture, overlay, 
	every edge/.append style = { ->, thick, >=stealth, dashed, line width = 1pt },
	every node/.append style = { align = center, font = \bfseries}]
		\node [above = of sfs] (1) {(1)};
		\node [above right = of fs] (2) {(2)};
		\node [below = of var1] (3) {(3)};
		\node [right = of lr] (4) {(4)};
		
		\draw (1.south) edge (sfs.north);
		\draw (2.south) edge (fs.north);
		\draw (3.north) edge (var1.south);
		\draw (3.north) edge (var2.south);
		\draw (4.west) edge (cond1.east);
		\draw (4.west) edge (cond2.east);
	\end{tikzpicture}
	\vspace{1em}
	\caption{The structure of a \its considered in this paper.}
	\label{fig:its-structure}
\end{figure}
%TODO: Explanation in text or in figure?
...
\begin{enumerate}[leftmargin=1]
	...
	\item[(3)] The variables of a function symbol denoting the change of the variables by applying the term rewriting rule
	\item[(4)] The conditional term of the form $\text{(in)equation}_1 \text{ \&\& } \dots \text{ \&\& } \text{(in)equation}_m$ $m \in \mathbb{N}$, where $\text{(in)equation}_i$ contains only $v_j$ $1 \le j \le n$ and is in standard integer form.\footnote{The standard form has the following pattern: $ a_1*v_1 + \dots + a_n*v_n \circ c$, where $a_i , v_i, c \in \mathbb{Z}$ and $\circ \in \{<,>\}$. Also it is important that $a_i * v_i$ has this order and not $v_i*a_i$}
	...
	\item[(line 2)] Such a rule is a self-looping rule considered within this approach to define further computations. Other looping rules will be presented in %TODO: REF related work.
\end{enumerate} 
...
\begin{definition}[LOOP]
	The \loopt is defined as a tuple $(A, b)$, where $A$ is the \iterationmatrix and $b$ the \iterationconstants of an \its. %TODO: REF TO LOOP
\end{definition}
...
%\begin{theorem}
%	If such a \gna $a$ for a program $p$ exists, then $p$ does not terminate. \cite{leike2014geometric} 
%\end{theorem}
...
\begin{figure}
	\begin{tikzpicture}[
			scale=0.6,
			every edge/.append style = { ->, thick, >=stealth, dashed, line width = 1pt },
			every node/.append style = {shape = rectangle, draw, rounded corners} ]
