...
\section{Integer Term Rewriting System(\its)}
\label{sec:its}
...
Through the in \autoref{sec:its} described procedure we receive the to \autoref{fig:structure-example-java} equivalent \its shown in \autoref{fig:structure-example-TRS}. As we can see the original program can be recognized quite easily. The first rule in line 1 denotes the \stem, while the second line equals the loop \loopt.

\begin{figure}[H]
	\begin{lstlisting}[linewidth=1.4\textwidth, escapechar = !]
!$\overbrace{f_1	     -> f_2(1+3*c,2)   :|: c>2 \text{ \&\& } 8<3*c}^{\text{\stem}}$!
...
\begin{figure}
	\begin{tikzpicture}[
		...
		\node (L) at (0,5) {\begin{tabular}{cc} mathematical expression: \\ 2*x+4*y \end{tabular} };
		\node (M) at (0,0) {\begin{tabular}{cc} reverse polish notation: \\ +*x2*y4 \end{tabular} };
		\node (R) at (10,2) {\includegraphics[width=.5\textwidth]{src/diagram/RPNExample.png}};
