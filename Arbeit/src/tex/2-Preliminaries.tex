...
we finally receive the equivalent \its shown in \autoref{fig:structure-example-TRS}. As we can see the original program can be recognized quite easily. The first rule in line 1 denotes the \stem, while the second line equals the loop \loopt.
...
\begin{figure}
	\begin{tikzpicture}[
		...
		\node (L) at (0,5) {\begin{tabular}{cc} mathematical expression: \\ 2*(x+4) \end{tabular} };
		\node (M) at (0,0) {\begin{tabular}{cc} reverse polish notation: \\ *2+x4 \end{tabular} };
