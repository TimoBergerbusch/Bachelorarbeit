...
\begin{definition}[standard guard form]
	...
	\end{figure}
\end{definition} 
The condition given be the \seg is one rule $r$ , which represents a set $G$  in standard guard form and 
\begin{center}
...
\begin{definition}[strict guard form]
	...
	$g := \varphi \le c$, with
	$\varphi$ in \stdLinInt and $c \in \mathbb{Z}$ subtracted by a possible constant $c_\varphi$ inherited from $\varphi$
\end{definition}

To transfer a guard from \stdG to \strG we have to apply the two following steps:
\begin{enumerate}
	\item \textbf{rewrite equations} \newline
		If the guards contain a guard with the symbol $=$ we have to rewrite the "new" variable. To define, which are new and substitute these we perform the following algorithm:
		\begin{algorithm}[H]
...
\begin{example}
	...
	\hspace*{1cm}$v_1 + v_2 > 3 \>\&\&\> v_1 > 6 \>\&\&\> 3 * v_1 > 20 \>\&\&\> 5 + v_3 = 2 * v_2 \>\&\&\> v_3 < -10$\newline
	which lead to the set $G$:\newline
	\hspace*{1cm}$\{v_1 + v_2 > 3\text{, } v_1 > 6 \text{, } 3 * v_1 > 20 \text{, } 5 + v_3 = 2 * v_2 \text{, } v_3 < -10\}$\newline	
...
\begin{example}
	...
	This example is based in the \its  from \autoref{fig:structure-example-TRS} in combination with the \autoref{ex:derivation-guard} providing $V_{sub}$. \newline
...
\begin{figure}
	...
	\draw[considered] (Plus.south)  -- ++(0,-0.4) -| (Times1.north) node [pos = 0.4, above, font=\footnotesize]{left};
	\draw[query] (Plus.south)  -- ++(0,-0.4) -| (var2.north) node [pos = 0.4, above, font=\footnotesize]{right};
